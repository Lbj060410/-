...
BarcodeData Code39Reader::decodePattern(int rowNumber, PatternView& next, std::unique_ptr<RowReader::DecodingState>&) const
{
	...
	if (format == BarcodeFormat::None && _opts.hasFormat(BarcodeFormat::PZN) && IsPZN(txt)) {
		format = BarcodeFormat::PZN;
	}
	if (format == BarcodeFormat::None && _opts.hasFormat(BarcodeFormat::Code32)) {
	...
	if (format == BarcodeFormat::None) {
		...
		if (_opts.hasFormat(BarcodeFormat::Code39Ext) && std::ranges::find_first_of(txt, shiftChars) != txt.end()) {
			auto fullASCII = DecodeCode39AndCode93FullASCII(txt, shiftChars);
			if (!fullASCII.empty()) {
				txt = std::move(fullASCII);
				format = BarcodeFormat::Code39Ext;
			}
		}

		if (format == BarcodeFormat::None && _opts.hasFormat(BarcodeFormat::Code39Std))
			format = BarcodeFormat::Code39;

		if (hasValidChecksum)
			txt.push_back(lastChar);

		symbologyIdentifier.modifier += (hasValidChecksum ? 1 : 0) + (format == BarcodeFormat::Code39Ext ? 4 : 0);
	}

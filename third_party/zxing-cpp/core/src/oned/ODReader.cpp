...
Reader::Reader(const ReaderOptions& opts) : ZXing::Reader(opts)
{
	...
	if (opts.hasAnyFormat(BarcodeFormat::EANUPC))
		_readers.emplace_back(new MultiUPCEANReader(opts));

	if (opts.hasAnyFormat(BarcodeFormat::Code39))
		_readers.emplace_back(new Code39Reader(opts));
	if (opts.hasAnyFormat(BarcodeFormat::Code93))
		_readers.emplace_back(new Code93Reader(opts));
	if (opts.hasAnyFormat(BarcodeFormat::Code128))
		_readers.emplace_back(new Code128Reader(opts));
	if (opts.hasAnyFormat(BarcodeFormat::ITF))
		_readers.emplace_back(new ITFReader(opts));
	if (opts.hasAnyFormat(BarcodeFormat::Codabar))
		_readers.emplace_back(new CodabarReader(opts));
	if (opts.hasFormat(BarcodeFormat::DataBar | BarcodeFormat::DataBarOmni | BarcodeFormat::DataBarStk |
					   BarcodeFormat::DataBarStkOmni))
		_readers.emplace_back(new DataBarReader(opts));
	if (opts.hasFormat(BarcodeFormat::DataBar | BarcodeFormat::DataBarExp | BarcodeFormat::DataBarExpStk))
		_readers.emplace_back(new DataBarExpandedReader(opts));
	if (opts.hasFormat(BarcodeFormat::DataBar | BarcodeFormat::DataBarLtd))
		_readers.emplace_back(new DataBarLimitedReader(opts));
	if (opts.hasAnyFormat(BarcodeFormat::DXFilmEdge))
